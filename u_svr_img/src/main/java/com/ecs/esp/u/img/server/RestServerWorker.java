package com.ecs.esp.u.img.server;
...
public class RestServerWorker extends EThreadServer {	
	...
	protected JSONMessage SetMessageData(JSONMessage msg) {
        if (msg.getMessageID() == RESTMessage.REQ_REST_PICTURE_SYNC) {
            msg.SetParamJson("TITLE", EnumBulk.PICTURE_SYNC.getTitle());
            msg.SetParamJson("REQ_URL", EnumBulk.PICTURE_SYNC.getURL());
            return msg;
        }
		return msg;
	}
			...
			if(obj instanceof RESTStringData data) {
				...
				if(msg != null) {
                    ...
                    if (msg.getMessageID() == RESTMessage.REQ_REST_PICTURE_SYNC) {
                        ...
                        EThread.postMessage(Define.TID_DB_WORKER, SetMessageData(msg));
                    } else {
     ...
     ***************************************************************************************************/
    private void syncComponent(String logTag,
                               String... keyValuePairs) {
        syncComponent(0, 0, logTag, keyValuePairs);
    }
    /****************************************************************************************************
	 * syncComponent : 동기화 처리
	 ****************************************************************************************************/
	private void syncComponent(int reqTID,
							   int requestType,
							   String logTag,
							   String... keyValuePairs) {
		try {
	...
	protected void onTimer(int timerID) {
		try {
			...
			if (syncTimer.checkSync()) {
				UtilLog.i(getClass(), "#############################################################");
				UtilLog.i(getClass(), "#			사진동기화 자동시작								   ");
				UtilLog.i(getClass(), "#############################################################");

				syncComponent(Define.TID_DB_WORKER,
						RESTMessage.REQ_REST_PICTURE_SYNC,
						"[일배치 동기화][" + EnumBulk.PICTURE_SYNC.getTitle() + "]",
						"TITLE",	EnumBulk.PICTURE_SYNC.getTitle(),
						"REQ_URL",  EnumBulk.PICTURE_SYNC.getURL());
